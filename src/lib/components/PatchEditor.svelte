<script lang="ts">
	...
	const {
		...
		initialPatch?: Patch;
		handleSavePatch?: (patch: Patch) => void;
	} = $props();
	...
	const handleSourceImageLoad = async (imageData: ImageData) => {
	...
	const handleUpdateBehavior = async (nodeIndex: number, behavior: Behavior) => {
		const actualIndex = Math.floor(nodeIndex / 2);
		await updateBehaviorAt(editorState, actualIndex, behavior);
		renderTrigger = !renderTrigger;
	};

	const isAdjustmentBehavior = (behavior: Behavior): boolean => {
		return (
			behavior.type === 'hsl' ||
			behavior.type === 'rgb' ||
	...
	const handleDelete = async (nodeIndex: number) => {

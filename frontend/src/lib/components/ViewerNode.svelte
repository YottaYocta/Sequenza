<script lang="ts">
	 ...
	 */
	const renderCanvas: Attachment<HTMLCanvasElement> = (canvas: HTMLCanvasElement) => {
...
>
	<span class="absolute top-0 elft-0 -translate-1/2 w-2 h-2 bg-black flex items-center">
	...
	{:else if output.type === 'image'}
		<div class="w-full h-min justify-start items-center flex gap-2">
			<button onclick={copyImageToClipboard} class="button-1"> COPY IMAGE </button>
	...
	{:else if output.type === 'svg'}
		...
		<div class="w-full h-full flex items-center justify-center p-2">
			<svg
				viewBox="{output.data.viewBox.x} {output.data.viewBox.y} {output.data.viewBox.width} {output
					.data.viewBox.height}"
